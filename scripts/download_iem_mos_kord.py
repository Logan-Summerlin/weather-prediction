...
Downloads GFS MOS and NAM MOS historical forecasts for KORD (Chicago O'Hare)
from IEM, extracts day-ahead max temperature forecasts, and saves processed data.

Data source: https://mesonet.agron.iastate.edu/cgi-bin/request/mos.py

Usage:
    python scripts/download_iem_mos_kord.py
"""
...
MOS_DATA_DIR = os.path.join(PROJECT_ROOT, "data", "chicago", "mos")
...
def download_mos_chunk(
    ...
    ----------
    station : str
        4-character station identifier (e.g., KORD).
    model : str
...
def main() -> None:
    """Main entry point: download, process, and save MOS data for KORD."""
    ...
    gfs_out_path = os.path.join(MOS_DATA_DIR, "gfs_mos_kord.csv")
    nam_out_path = os.path.join(MOS_DATA_DIR, "nam_mos_kord.csv")
    combined_out_path = os.path.join(MOS_DATA_DIR, "combined_mos_kord.csv")
