#!/usr/bin/env python3
"""
Philadelphia Advanced Model Benchmark Script.

Trains and evaluates advanced neural network models for Philadelphia (KXHIGHPHL)
...
All models use:
  ...
  - Up to 55 surrounding weather stations within 200mi of PHL
...
Usage:
    python scripts/run_phl_advanced_benchmark.py
"""
...
city_config = get_city_runtime_config("phl")
...
def load_and_split_phl_data(
    processed_dir: str,
) -> tuple:
    """Load Philadelphia data and split chronologically by year.
    ...
    if not os.path.isdir(processed_dir):
        raise FileNotFoundError(
            ...
            "Run scripts/run_phl_preprocessing.py first."
        )
    ...
    if not X_full_parts:
        raise FileNotFoundError(
            f"No feature/target CSV files found in {processed_dir}."
        )

    X_full = pd.concat(X_full_parts, axis=0).sort_index()
    y_full = pd.concat(y_full_parts, axis=0).sort_index()
    y_full.name = "PHL_TMAX"

    # Re-split by date
    train_mask = (X_full.index >= "2000-01-01") & (X_full.index < "2022-01-01")
    ...
    logger.info(
        "Loaded and re-split PHL data: train=%d (%s to %s), "
    ...
    X_val_scaled = pd.DataFrame(
        scaler.transform(X_val), index=X_val.index, columns=feature_cols
    )
...
def run_ridge_baseline(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    bucket_edges = get_city_config("phl").bucket_edges
...
def plot_brier_comparison(results, save_path, title="Philadelphia"):
...
def main():
    logger.info("=" * 70)
    logger.info("Philadelphia ADVANCED Model Benchmark (KXHIGHPHL)")
    ...
    # --- Load data ---
    X_train, X_val, X_test, y_train, y_val, y_test = load_and_split_phl_data(
        processed_dir
    )
    ...
    fa_result = train_model(
        ...
        model_type="standard",
        lr=0.001, max_epochs=300, patience=25, batch_size=64,
        loss_type="crps_mae",
    )
    ...
    plot_feature_importance(
        ...
        os.path.join(results_dir, "phl_feature_importance.png"),
    )
    ...
    baseline_all, _ = compute_mos_baseline(
    ...
    mos_model = MOSCorrectionNet(
        n_features=n_feat_enh, hidden_sizes=[128, 64, 32], dropout=0.15,
    )
    ...
    # Save artifacts
    summary_df.to_csv(os.path.join(results_dir, "phl_advanced_benchmark_summary.csv"), index=False)
    ...
    with open(os.path.join(results_dir, "phl_advanced_benchmark_detail.json"), "w") as f:
        json.dump(detail, f, indent=2, default=str)

    plot_brier_comparison(
        all_results,
        os.path.join(results_dir, "phl_advanced_brier_comparison.png"),
        title="Philadelphia",
    )
    ...
    with open(os.path.join(results_dir, "phl_advanced_benchmark_metadata.json"), "w") as f:
        json.dump(metadata, f, indent=2)

    logger.info("=" * 70)
    logger.info("Philadelphia Advanced Benchmark Complete")
