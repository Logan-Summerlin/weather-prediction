...
Results are saved to results/atlanta/backtest/.

Usage:
    python scripts/run_atl_backtest.py
"""
...
def generate_atl_market_data(
    ...
    logger.info(
        "Generated ATL market data: %d rows across %d days, %d buckets",
...
def run_ev_gated_backtest(
    ...
    ----------
    market_df : pd.DataFrame
        Market dataset from generate_atl_market_data().
    ev_threshold : float
...
def plot_pnl_curve(
    ...
    initial_bankroll: float,
    output_path: str,
    title: str = "ATL Backtest: Cumulative P&L",
) -> None:
...
def plot_brier_comparison(
    ...
    market_brier: float,
    output_path: str,
    title: str = "ATL Model vs Market Brier Score",
) -> None:
...
def plot_seasonal_pnl(
    ...
    output_path: str,
    title: str = "ATL Backtest: Seasonal P&L Breakdown",
) -> None:
...
def plot_trade_ev_distribution(
    trades: list[dict],
    output_path: str,
    title: str = "ATL Backtest: Trade EV Distribution",
) -> None:
...
def load_calibrated_predictions(results_dir: str) -> pd.DataFrame:
    """Load calibrated ATL synthesis predictions.

    Looks for the synthesis predictions CSV produced by
    run_atl_synthesis_calibration.py.  Falls back to base predictions
    if synthesis not found.

    Parameters
    ----------
    results_dir : str
        Path to results/atlanta/ directory.
    ...
    raise RuntimeError(
        ...
        f"Run the ATL synthesis/calibration script first to generate real "
...
def main() -> None:
    ...
    if not np.isnan(metrics.get("model_brier", float("nan"))):
        plot_brier_comparison(
            ...
            title="ATL Model vs Market: Brier Score",
        )
    ...
    if backtest_result["trades"]:
        plot_trade_ev_distribution(
            ...
            title="ATL Backtest: Trade EV Distribution",
        )
