...
Models benchmarked:
  - Persistence baseline   (yesterday's ATL TMAX)
...
Results are saved to results/atlanta/.

Usage:
    python scripts/run_atl_benchmark.py
"""
...
def load_processed_atl_data(processed_dir: str) -> tuple:
    """Load preprocessed Atlanta CSV files from disk.
    ...
    if not os.path.isdir(processed_dir):
        raise FileNotFoundError(
            ...
            "Run scripts/run_atl_preprocessing.py first."
        )
    ...
    logger.info("Loaded ATL processed data from %s", processed_dir)
...
def plot_brier_comparison(
    ...
    ax.set_title("Atlanta Model Benchmark: Contract Brier Scores")
...
def plot_seasonal_brier(
    ...
    ax.set_title("Atlanta Benchmark: Seasonal Contract Brier Scores")
...
def main():
    ...
    bucket_edges = atl.bucket_edges
    bucket_labels = atl.bucket_labels
    ...
    nn_model_path = os.path.join(atl.models_dir, "heteroscedastic_nn_atl.pt")
    os.makedirs(atl.models_dir, exist_ok=True)
    ...
    # --- Save results ---
    summary_path = os.path.join(results_dir, "atl_benchmark_summary.csv")
    summary_df.to_csv(summary_path, index=False)
    logger.info("Saved summary to %s", summary_path)

    # Detailed results JSON
    detail_path = os.path.join(results_dir, "atl_benchmark_detail.json")
    ...
    if nn_res.get("history"):
        ...
        history_path = os.path.join(results_dir, "atl_nn_training_history.csv")
    ...
    plot_brier_comparison(
        all_results,
        os.path.join(results_dir, "atl_brier_comparison.png"),
    )
    plot_seasonal_brier(
        seasonal_all,
        os.path.join(results_dir, "atl_seasonal_brier.png"),
    )
    ...
    metadata_path = os.path.join(results_dir, "atl_benchmark_metadata.json")
    with open(metadata_path, "w") as f:
        json.dump(metadata, f, indent=2)
    logger.info("Saved metadata to %s", metadata_path)

    logger.info("=" * 70)
    logger.info("Atlanta Benchmark Complete")
