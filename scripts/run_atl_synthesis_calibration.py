#!/usr/bin/env python3
"""
Atlanta Synthesis Model Training and Calibration Sweep.
...
Usage:
    python scripts/run_atl_synthesis_calibration.py
"""
...
def load_base_predictions(results_dir: str) -> pd.DataFrame:
    """Load base model predictions from the ATL benchmark results.

    Looks for the benchmark summary JSON and per-model CSV prediction files
    produced by run_atl_benchmark.py.  Falls back to generating synthetic
    predictions from ATL climatology if benchmark outputs are not found.
    ...
    raise RuntimeError(
        ...
        f"Run the ATL benchmark script (run_atl_benchmark.py) first to "
...
def train_synthesis_mlp(
    ...
    batch_size: int = 64,
    lr: float = 0.001,
    model_name: str = "U_atl_synthesis",
) -> dict:
...
def plot_brier_comparison(
    ...
    title: str = "ATL Calibration Sweep: Brier Scores",
    save_path: Optional[str] = None,
) -> None:
...
def run_calibration_sweep(
    ...
    for method_name, info in results.items():
        ...
        plot_reliability(
            rel_data,
            title=f"ATL {method_name}: Reliability",
    ...
    plot_brier_comparison(
        method_briers,
        title="ATL Calibration Sweep: OOS Brier Scores",
...
def main() -> None:
    """Run the full ATL synthesis model training and calibration sweep."""
    ...
    logger.info("=" * 70)
    logger.info("Atlanta Synthesis Model Training & Calibration Sweep")
    ...
    if history:
        ...
        fig.suptitle("ATL Synthesis Model: Training Curves", fontsize=14, fontweight="bold")
        fig.tight_layout()
    ...
    logger.info("=" * 70)
    logger.info("ATL Synthesis Calibration Sweep Complete")
