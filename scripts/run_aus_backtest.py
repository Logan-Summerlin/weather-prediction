#!/usr/bin/env python3
"""
Austin KXHIGHAUS Market Backtest.

Simulates Kalshi KXHIGHAUS market conditions and runs an EV-gated trading
...
Results are saved to results/austin/backtest/.

Usage:
    python scripts/run_aus_backtest.py
"""
...
def generate_aus_market_data(
    ...
    """Simulate KXHIGHAUS market data for backtesting.
    ...
    logger.info(
        "Generated AUS market data: %d rows across %d days, %d buckets",
...
def plot_pnl_curve(
    ...
    initial_bankroll: float,
    output_path: str,
    title: str = "AUS Backtest: Cumulative P&L",
) -> None:
...
def plot_brier_comparison(
    ...
    market_brier: float,
    output_path: str,
    title: str = "AUS Model vs Market Brier Score",
) -> None:
...
def plot_seasonal_pnl(
    ...
    output_path: str,
    title: str = "AUS Backtest: Seasonal P&L Breakdown",
) -> None:
...
def plot_trade_ev_distribution(
    trades: list[dict],
    output_path: str,
    title: str = "AUS Backtest: Trade EV Distribution",
) -> None:
...
def load_calibrated_predictions(results_dir: str) -> pd.DataFrame:
    """Load calibrated AUS synthesis predictions.

    Looks for the synthesis predictions CSV produced by
    run_aus_synthesis_calibration.py.  Falls back to generating synthetic
    ...
    raise RuntimeError(
        ...
        f"Run the AUS synthesis/calibration script first to generate real "
...
def main() -> None:
    """Run the full AUS KXHIGHAUS market backtest."""
    ...
    logger.info("=" * 70)
    logger.info("Austin KXHIGHAUS Market Backtest")
    ...
    if len(backtest_result["bankroll_series"]) > 0:
        plot_pnl_curve(
            ...
            title="AUS KXHIGHAUS Backtest: Cumulative P&L",
        )
    ...
    if not np.isnan(metrics.get("model_brier", float("nan"))):
        plot_brier_comparison(
            ...
            title="AUS Model vs Market: Brier Score",
        )
    ...
    if metrics.get("seasonal"):
        plot_seasonal_pnl(
            ...
            title="AUS KXHIGHAUS Backtest: Seasonal P&L",
        )
