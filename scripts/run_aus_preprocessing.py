#!/usr/bin/env python3
"""
Austin Preprocessing Pipeline.

Processes raw GHCN-Daily station CSVs into training-ready feature matrices
for the Austin temperature prediction model.

Pipeline steps:
  1. Load all AUS station CSVs from data/austin/raw/
...
Usage:
    python scripts/run_aus_preprocessing.py
"""
...
def load_chi_stations(raw_dir: str) -> dict[str, pd.DataFrame]:
    ...
    logger.info("Loaded %d / %d AUS station datasets",
...
def filter_chi_stations_by_completeness(
    ...
    min_completeness : float, optional
        ...
        Defaults to config_austin.MIN_COMPLETENESS (0.80).
...
def create_chi_target_and_features(
    ...
    # Target: AUS TMAX on day t
    target = merged_df[target_col].copy()
    target.name = "AUS_TMAX"
    ...
    logger.info("Created AUS features: %d rows x %d columns", *features.shape)
    logger.info("Target (AUS_TMAX): %d non-missing values", target.notna().sum())
...
def generate_chi_preprocessing_report(
    ...
    """Generate a human-readable preprocessing report for AUS.
    ...
    lines = [
        "=" * 70,
        "Austin Temperature Prediction -- Preprocessing Report",
        "=" * 70,
        "",
...
def main():
    """Execute the full Austin preprocessing pipeline."""
    logger.info("=" * 60)
    ...
    # 13. Summary
    logger.info("=" * 60)
    logger.info("Austin Preprocessing Complete")
    logger.info("=" * 60)
    ...
    logger.info("Next step: run scripts/run_aus_benchmark.py")
