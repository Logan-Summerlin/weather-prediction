#!/usr/bin/env python3
"""
Chicago KXHIGHCHI Market Backtest.

Simulates Kalshi KXHIGHCHI market conditions and runs an EV-gated trading
...
Results are saved to results/chicago/backtest/.
...
def generate_chi_market_data(
    ...
    """Simulate KXHIGHCHI market data for backtesting.
    ...
    logger.info(
        "Generated CHI market data: %d rows across %d days, %d buckets",
...
def plot_pnl_curve(
    ...
    initial_bankroll: float,
    output_path: str,
    title: str = "CHI Backtest: Cumulative P&L",
) -> None:
...
def plot_brier_comparison(
    ...
    market_brier: float,
    output_path: str,
    title: str = "CHI Model vs Market Brier Score",
) -> None:
...
def plot_seasonal_pnl(
    ...
    output_path: str,
    title: str = "CHI Backtest: Seasonal P&L Breakdown",
) -> None:
...
def plot_trade_ev_distribution(
    trades: list[dict],
    output_path: str,
    title: str = "CHI Backtest: Trade EV Distribution",
) -> None:
...
def load_calibrated_predictions(results_dir: str) -> pd.DataFrame:
    """Load calibrated CHI synthesis predictions.

    Looks for the synthesis predictions CSV produced by
    run_chi_synthesis_calibration.py.  Falls back to generating synthetic
    predictions if not found.

    Parameters
    ----------
    results_dir : str
        Path to results/chicago/ directory.
    ...
    raise RuntimeError(
        ...
        f"Run the CHI synthesis/calibration script first to generate real "
...
def main() -> None:
    """Run the full CHI KXHIGHCHI market backtest."""
    ...
    logger.info("=" * 70)
    logger.info("Chicago KXHIGHCHI Market Backtest")
    ...
    logger.info("Step 2: Generating simulated KXHIGHCHI market data ...")
    ...
    if len(backtest_result["bankroll_series"]) > 0:
        plot_pnl_curve(
            ...
            title="CHI KXHIGHCHI Backtest: Cumulative P&L",
        )
    ...
    if not np.isnan(metrics.get("model_brier", float("nan"))):
        plot_brier_comparison(
            ...
            title="CHI Model vs Market: Brier Score",
        )
    ...
    if metrics.get("seasonal"):
        plot_seasonal_pnl(
            ...
            title="CHI KXHIGHCHI Backtest: Seasonal P&L",
        )
    ...
    if backtest_result["trades"]:
        plot_trade_ev_distribution(
            ...
            title="CHI Backtest: Trade EV Distribution",
        )

    # ---- Final summary ----
    logger.info("=" * 70)
    logger.info("CHI KXHIGHCHI Backtest Complete")
