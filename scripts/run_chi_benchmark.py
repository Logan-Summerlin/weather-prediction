...
(mu, sigma) which is mapped to KXHIGHCHI contract bucket probabilities
...
def plot_brier_comparison(
    ...
    ax.set_title("Chicago Model Benchmark: Test-Set Brier Scores")
...
def plot_seasonal_brier(
    ...
    ax.set_title("Chicago Benchmark: Seasonal Brier Scores (Test Set)")
...
def main():
    """Run the full Chicago model benchmark."""
    logger.info("=" * 70)
    logger.info("Chicago Model Benchmark (KXHIGHCHI)")
    logger.info("=" * 70)
    ...
    per_bucket_path = os.path.join(results_dir, "chi_per_bucket_brier.csv")
