#!/usr/bin/env python3
"""
Chicago Model Benchmark Script.

Trains and evaluates a suite of models for Chicago (KXHIGHCHI)
temperature prediction, computing Brier scores on held-out OOS data
using CHI bucket definitions from city_config.

Models benchmarked:
  - Persistence baseline   (yesterday's CHI TMAX)
...
def load_processed_chi_data(processed_dir: str) -> tuple:
    """Load preprocessed Chicago CSV files from disk.
...
def load_kalshi_data_chi() -> pd.DataFrame:
    """Load Kalshi settlement + pre-settlement data for Chicago."""
    ...
    if presettlement_path.exists():
        ...
        logger.info("Loaded Kalshi CHI (pre-settlement): %d rows, %d dates",
                     len(merged), merged["date"].nunique())
        return merged
    logger.info("Loaded Kalshi CHI (settlement only): %d rows", len(settled))
    return settled
...
def plot_brier_comparison(
    ...
    ax.set_title("Chicago Model Benchmark: Contract Brier Scores")
...
def plot_seasonal_brier(
    ...
    ax.set_title("Chicago Benchmark: Seasonal Contract Brier Scores")
...
def main():
    """Run the full Chicago model benchmark with contract-level Brier."""
    logger.info("=" * 70)
    logger.info("Chicago Model Benchmark (KXHIGHCHI) — Contract Brier")
    logger.info("=" * 70)
    ...
    # --- Save results ---
    summary_path = os.path.join(results_dir, "chi_benchmark_summary.csv")
    summary_df.to_csv(summary_path, index=False)
    logger.info("Saved summary to %s", summary_path)

    # Detailed results JSON
    detail_path = os.path.join(results_dir, "chi_benchmark_detail.json")
    ...
    if nn_res.get("history"):
        ...
        history_path = os.path.join(results_dir, "chi_nn_training_history.csv")
    ...
    plot_brier_comparison(
        all_results,
        os.path.join(results_dir, "chi_brier_comparison.png"),
    )
    plot_seasonal_brier(
        seasonal_all,
        os.path.join(results_dir, "chi_seasonal_brier.png"),
    )
    ...
    metadata_path = os.path.join(results_dir, "chi_benchmark_metadata.json")
    with open(metadata_path, "w") as f:
        json.dump(metadata, f, indent=2)
    logger.info("Saved metadata to %s", metadata_path)

    logger.info("=" * 70)
    logger.info("Chicago Benchmark Complete")
