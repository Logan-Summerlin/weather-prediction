#!/usr/bin/env python3
"""
Chicago Preprocessing Pipeline.

Processes raw GHCN-Daily station CSVs into training-ready feature matrices
for the Chicago temperature prediction model.

Pipeline steps:
  1. Load all CHI station CSVs from data/chicago/raw/
  2. Merge into a wide DataFrame (station_id_TMAX, station_id_TMIN, ...)
  3. Filter stations below completeness threshold
  4. Create target (CHI TMAX) and lagged surrounding-station features
...
# ===========================================================================
# CHI-Specific Functions
# ===========================================================================

def load_chi_stations(raw_dir: str) -> dict[str, pd.DataFrame]:
    """Load all CHI station CSVs from the raw data directory.
    ...
    logger.info("Loaded %d / %d CHI station datasets",
...
def filter_chi_stations_by_completeness(
    ...
    Uses CHI config defaults.  The target station (USW00094846) is never
    ...
    min_completeness : float, optional
        ...
        Defaults to config_chicago.MIN_COMPLETENESS (0.80).
...
def create_chi_target_and_features(
    ...
    """Create the CHI target variable and lagged feature columns.

    Target: CHI TMAX on day t (O'Hare International Airport).
    Features: All surrounding-station values shifted by +1 (so row for day t
    contains surrounding-station data from day t-1).

    NOTE: Chicago-specific lake-effect features (e.g., Lake Michigan wind
    ...
    # Target: CHI TMAX on day t
    target = merged_df[target_col].copy()
    target.name = "CHI_TMAX"
    ...
    logger.info("Created CHI features: %d rows x %d columns", *features.shape)
    logger.info("Target (CHI_TMAX): %d non-missing values", target.notna().sum())
...
def chronological_split_chi(
    ...
    train_ratio : float, optional
        Fraction for training. Defaults to config_chicago.TRAIN_RATIO.
    val_ratio : float, optional
        Fraction for validation. Defaults to config_chicago.VAL_RATIO.
...
def generate_chi_preprocessing_report(
    ...
    """Generate a human-readable preprocessing report for CHI.
    ...
    lines = [
        "=" * 70,
        "Chicago Temperature Prediction -- Preprocessing Report",
        "=" * 70,
        "",
...
def main():
    """Execute the full Chicago preprocessing pipeline."""
    logger.info("=" * 60)
    logger.info("Chicago Preprocessing Pipeline")
    logger.info("=" * 60)
    ...
    # 1. Load station data
    logger.info("Step 1: Loading CHI station data from %s", raw_dir)
    station_data = load_chi_stations(raw_dir)
    if not station_data:
        logger.error("No station data found. Run run_chi_data_collection.py first.")
        return
    ...
    # 13. Summary
    logger.info("=" * 60)
    logger.info("Chicago Preprocessing Complete")
    logger.info("=" * 60)
    ...
    logger.info("Next step: run scripts/run_chi_benchmark.py")
