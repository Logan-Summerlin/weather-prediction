#!/usr/bin/env python3
"""
Chicago Synthesis Model Training and Calibration Sweep.
...
bucket probabilities for KXHIGHCHI.
...
def load_base_predictions(results_dir: str) -> pd.DataFrame:
    """Load base model predictions from the CHI benchmark results.

    Looks for the benchmark summary JSON and per-model CSV prediction files
    produced by run_chi_benchmark.py.  Falls back to generating synthetic
    predictions from CHI climatology if benchmark outputs are not found.
...
def plot_brier_comparison(
    ...
    title: str = "CHI Calibration Sweep: Brier Scores",
    save_path: Optional[str] = None,
) -> None:
...
def run_calibration_sweep(
    ...
    for method_name, info in results.items():
        ...
        plot_reliability(
            rel_data,
            title=f"CHI {method_name}: Reliability",
    ...
    plot_brier_comparison(
        method_briers,
        title="CHI Calibration Sweep: OOS Brier Scores",
...
def main() -> None:
    """Run the full CHI synthesis model training and calibration sweep."""
    cfg = get_city_config("chi")
    ...
    logger.info("=" * 70)
    logger.info("Chicago Synthesis Model Training & Calibration Sweep")
    ...
    if history:
        ...
        fig.suptitle("CHI Synthesis Model: Training Curves", fontsize=14, fontweight="bold")
        fig.tight_layout()
    ...
    logger.info("=" * 70)
    logger.info("CHI Synthesis Calibration Sweep Complete")
