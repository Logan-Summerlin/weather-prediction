#!/usr/bin/env python3
"""
Philadelphia KXHIGHPHL Market Backtest.

Simulates Kalshi KXHIGHPHL market conditions and runs an EV-gated trading
...
Results are saved to results/philadelphia/backtest/.
...
def generate_phl_market_data(
    ...
    """Simulate KXHIGHPHL market data for backtesting.
    ...
    logger.info(
        "Generated PHL market data: %d rows across %d days, %d buckets",
        len(df), n_days, n_buckets,
    )
    return df
...
def run_ev_gated_backtest(
    ...
    ----------
    market_df : pd.DataFrame
        Market dataset from generate_phl_market_data().
    ev_threshold : float
...
def plot_pnl_curve(
    ...
    initial_bankroll: float,
    output_path: str,
    title: str = "PHL Backtest: Cumulative P&L",
) -> None:
...
def plot_brier_comparison(
    ...
    market_brier: float,
    output_path: str,
    title: str = "PHL Model vs Market Brier Score",
) -> None:
...
def plot_seasonal_pnl(
    ...
    output_path: str,
    title: str = "PHL Backtest: Seasonal P&L Breakdown",
) -> None:
...
def plot_trade_ev_distribution(
    trades: list[dict],
    output_path: str,
    title: str = "PHL Backtest: Trade EV Distribution",
) -> None:
...
def load_calibrated_predictions(results_dir: str) -> pd.DataFrame:
    """Load calibrated PHL synthesis predictions.

    Looks for the synthesis predictions CSV produced by
    run_phl_synthesis_calibration.py.  Falls back to generating synthetic
    predictions if not found.

    Parameters
    ----------
    results_dir : str
        Path to results/philadelphia/ directory.
...
def main() -> None:
    """Run the full PHL KXHIGHPHL market backtest."""
    ...
    logger.info("=" * 70)
    logger.info("Philadelphia KXHIGHPHL Market Backtest")
    ...
    preds_df = load_calibrated_predictions(cfg.results_dir)
    ...
    logger.info("Step 2: Generating simulated KXHIGHPHL market data ...")
    market_df = generate_phl_market_data(
    ...
    if len(backtest_result["bankroll_series"]) > 0:
        plot_pnl_curve(
            ...
            title="PHL KXHIGHPHL Backtest: Cumulative P&L",
        )
    ...
    if not np.isnan(metrics.get("model_brier", float("nan"))):
        plot_brier_comparison(
            ...
            title="PHL Model vs Market: Brier Score",
        )
    ...
    if metrics.get("seasonal"):
        plot_seasonal_pnl(
            ...
            title="PHL KXHIGHPHL Backtest: Seasonal P&L",
        )
    ...
    if backtest_result["trades"]:
        plot_trade_ev_distribution(
            ...
            title="PHL Backtest: Trade EV Distribution",
        )

    # ---- Final summary ----
    logger.info("=" * 70)
    logger.info("PHL KXHIGHPHL Backtest Complete")
