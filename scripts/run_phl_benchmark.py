#!/usr/bin/env python3
"""
Philadelphia Model Benchmark Script.

Trains and evaluates a suite of models for Philadelphia (KXHIGHPHL)
temperature prediction, computing Brier scores on held-out OOS data
using PHL bucket definitions from city_config.

Models benchmarked:
  - Persistence baseline   (yesterday's PHL TMAX)
...
def load_processed_phl_data(processed_dir: str) -> tuple:
    """Load preprocessed Philadelphia CSV files from disk.
    ...
    logger.info("Loaded PHL processed data from %s", processed_dir)
...
def main():
    ...
    nn_model_path = os.path.join(phl.models_dir, "heteroscedastic_nn_phl.pt")
    os.makedirs(phl.models_dir, exist_ok=True)
    ...
    if nn_res.get("history"):
        ...
        history_path = os.path.join(results_dir, "phl_nn_training_history.csv")
    ...
    plot_brier_comparison(
        all_results,
        os.path.join(results_dir, "phl_brier_comparison.png"),
    )
    plot_seasonal_brier(
        seasonal_all,
        os.path.join(results_dir, "phl_seasonal_brier.png"),
    )
