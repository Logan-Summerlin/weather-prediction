#!/usr/bin/env python3
"""
Philadelphia Preprocessing Pipeline.

Processes raw GHCN-Daily station CSVs into training-ready feature matrices
for the Philadelphia temperature prediction model.

Pipeline steps:
  1. Load all PHL station CSVs from data/philadelphia/raw/
  2. Merge into a wide DataFrame (station_id_TMAX, station_id_TMIN, ...)
  3. Filter stations below completeness threshold
  4. Create target (PHL TMAX) and lagged surrounding-station features
  ...
  9. Save processed outputs to data/philadelphia/processed/

Key differences from NYC pipeline:
  - Uses config_philadelphia.ALL_STATIONS (PHL station network)
  - Target column: USW00013739_TMAX (PHL International Airport)
  - MIN_COMPLETENESS = 0.80 (vs 0.90 for NYC, to allow more stations)
  - Outputs saved to data/philadelphia/processed/
...
# ===========================================================================
# PHL-Specific Functions
# ===========================================================================

def load_phl_stations(raw_dir: str) -> dict[str, pd.DataFrame]:
    """Load all PHL station CSVs from the raw data directory.
...
def filter_phl_stations_by_completeness(
    ...
    min_completeness : float, optional
        ...
        Defaults to config_philadelphia.MIN_COMPLETENESS (0.80).
...
def create_phl_target_and_features(
    merged_df: pd.DataFrame,
) -> tuple[pd.DataFrame, pd.Series]:
    """Create the PHL target variable and lagged feature columns.

    Target: PHL TMAX on day t (Philadelphia International Airport).
    ...
    # Target: PHL TMAX on day t
    target = merged_df[target_col].copy()
    target.name = "PHL_TMAX"
    ...
    logger.info("Created PHL features: %d rows x %d columns", *features.shape)
    logger.info("Target (PHL_TMAX): %d non-missing values", target.notna().sum())
    return features, target


def chronological_split_phl(
    ...
    train_ratio : float, optional
        Fraction for training. Defaults to config_philadelphia.TRAIN_RATIO.
    val_ratio : float, optional
        Fraction for validation. Defaults to config_philadelphia.VAL_RATIO.
...
def generate_phl_preprocessing_report(
    merged_df: pd.DataFrame,
    ...
    """Generate a human-readable preprocessing report for PHL.
    ...
    lines = [
        "=" * 70,
        "Philadelphia Temperature Prediction -- Preprocessing Report",
        "=" * 70,
        "",
...
def main():
    """Execute the full Philadelphia preprocessing pipeline."""
    logger.info("=" * 60)
    logger.info("Philadelphia Preprocessing Pipeline")
    logger.info("=" * 60)
    ...
    # 1. Load station data
    logger.info("Step 1: Loading PHL station data from %s", raw_dir)
    station_data = load_phl_stations(raw_dir)
    if not station_data:
        logger.error("No station data found. Run run_phl_data_collection.py first.")
        return
    ...
    merged_filtered, dropped_stations = filter_phl_stations_by_completeness(merged)

    # 5. Create target and lagged features
    logger.info("Step 5: Creating target and lagged features")
    features, target = create_phl_target_and_features(merged_filtered)
    ...
    X_train, X_val, X_test, y_train, y_val, y_test = chronological_split_phl(
        features, target
    )
    ...
    report = generate_phl_preprocessing_report(
    ...
    # 13. Summary
    logger.info("=" * 60)
    logger.info("Philadelphia Preprocessing Complete")
    logger.info("=" * 60)
    ...
    logger.info("Next step: run scripts/run_phl_benchmark.py")
