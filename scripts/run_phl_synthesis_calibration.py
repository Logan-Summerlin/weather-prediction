#!/usr/bin/env python3
"""
Philadelphia Synthesis Model Training and Calibration Sweep.
...
bucket probabilities for KXHIGHPHL.
...
def load_base_predictions(results_dir: str) -> pd.DataFrame:
    """Load base model predictions from the PHL benchmark results.

    Looks for the benchmark summary JSON and per-model CSV prediction files
    produced by run_phl_benchmark.py.  Falls back to generating synthetic
    predictions from PHL climatology if benchmark outputs are not found.
    ...
    # Final fallback: generate synthetic predictions from PHL climatology
    logger.warning(
        "No benchmark predictions found in %s. "
        "Generating synthetic predictions from PHL climatology.",
        results_dir,
    )
...
def _generate_synthetic_base_predictions() -> pd.DataFrame:
    """Generate synthetic base model predictions from PHL climatology.
    ...
    logger.info(
        "Generated %d synthetic base predictions for PHL (%s to %s)",
...
def train_synthesis_mlp(
    ...
    batch_size: int = 64,
    lr: float = 0.001,
    model_name: str = "U_phl_synthesis",
) -> dict:
...
def plot_brier_comparison(
    ...
    title: str = "PHL Calibration Sweep: Brier Scores",
    save_path: Optional[str] = None,
) -> None:
...
def run_calibration_sweep(
    ...
    for method_name, info in results.items():
        ...
        plot_reliability(
            rel_data,
            title=f"PHL {method_name}: Reliability",
    ...
    plot_brier_comparison(
        method_briers,
        title="PHL Calibration Sweep: OOS Brier Scores",
...
def main() -> None:
    """Run the full PHL synthesis model training and calibration sweep."""
    cfg = get_city_config("phl")
    ...
    logger.info("=" * 70)
    logger.info("Philadelphia Synthesis Model Training & Calibration Sweep")
    ...
    if history:
        ...
        fig.suptitle("PHL Synthesis Model: Training Curves", fontsize=14, fontweight="bold")
        fig.tight_layout()
    ...
    logger.info("=" * 70)
    logger.info("PHL Synthesis Calibration Sweep Complete")
